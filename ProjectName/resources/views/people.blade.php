...
<?php
    ...
    if(isset($_GET['person'])){
        $person = $_GET['person']
        $results = DB::select('select * from people where peepID = :id LIMIT 1', ['id' => $person]);
        $notes = DB::select('select * from peepNotes where peepID = :id', ['id' => $results[0]->peepID]); //set two arrays person's details and a persons notes
    }
    else {
...
<?php
    ...
    function insertNote(){
        ...
        if (strlen($note) > 254){
            ...
            DB::insert('insert into peepNotes (note, peepID) values (?, ?)', [$_GET['note'], $results[0]->peepID]); //enter the data into the notes db
        }
    }
?>
@endsection
